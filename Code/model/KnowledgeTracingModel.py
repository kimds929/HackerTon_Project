...
# KnowledgeTracing
class KnowledgeTracing_Transformer_MidSchool(nn.Module):
    def __init__(self, knowledge_graph, KCs_size=12000, 
                 KCs_embed_dim=32, difficulty_embed_dim=8, ans_embed_dim = 8,
                 preKnowledgeWeight=0.7, postKnowledgeWeight=0.2,
                 preKnowledgeDepth=40, preKnowledgeCapa=150,
                 postKnowledgeDepth=40, postKnowledgeCapa=150,
                 output_dim=1,
                 num_heads=4, max_len=10000, n_blocks=3, device='cpu'):
        super().__init__()
        self.preKnowledgeWeight = preKnowledgeWeight
        self.postKnowledgeWeight = postKnowledgeWeight
        self.output_dim = output_dim
        ...
        # positional encoding
        self.transformer_embed_dim = self.KCs_graph_embedding.embed_dim + self.AnserEmbedding.embed_dim + self.difficulty_embedding.embed_dim

        self.positional_encoding = PositionalEncoding(self.transformer_embed_dim,
                                                    max_len=max_len, dropout=0)

        # transformer
        self.transformer_encoder_layer = nn.TransformerEncoderLayer(d_model=self.transformer_embed_dim, nhead=num_heads, dim_feedforward=self.transformer_embed_dim*2, dropout=0, batch_first=True)
    ...
    def forward(self, KCs, answer, difficulty):
        ...
        src_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1).bool().to(embedding_add_position.device)

        # transformer
        transformer_output = self.transformer_encoder(embedding_add_position, mask=src_mask)
...
# ktt = KnowledgeTracing_Transformer_MidSchool(knowledge_graph)
...
# kcs = torch.randint(0, num_tokens, (batch_size, max_seq_len))
# kcs = torch.tensor([[3,4,3],[10,24,5]])
# kcs = torch.tensor([[3,4,3,5],[10,24,5,7]])
# kcs = torch.tensor([3,4,3,5])
# kcs = torch.tensor(3)

# ans = torch.tensor(np.random.choice([1, 0, -1], size=tuple(kcs.shape))).type(torch.float32)
# diff = torch.rand_like(kcs.type(torch.float32))

# ans = torch.tensor([1,-1,0,1]).type(torch.float32)
# diff =torch.tensor([0.5,0.42,0.7,0.3]).type(torch.float32)
# ktt(kcs,ans, diff)
# ktt.predict(kcs,ans, diff)
# ktt.get_knowledge_state(kcs,ans, diff)
# ktt.get_sequential_knowledge_state(kcs,ans, diff)

# a.numpy()
# b = np.ones(1440) * np.inf
# b[:1434] = a.numpy().ravel()

# plt.imshow(b.reshape(45,-1), cmap='jet')
# plt.colorbar()

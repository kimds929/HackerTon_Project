...
def inference(data):
    # preprocessing
    kcs, ans, dif = data
    kcs_torch = torch.tensor(kcs, dtype=torch.int64).view(-1).to(device)
    ans_torch = torch.tensor(ans, dtype=torch.float32).view(-1).to(device)
    diff_torch = torch.tensor(dif, dtype=torch.float32).view(-1).to(device)
    
    # kt model
    state = kt_model.get_knowledge_state(kcs_torch, ans_torch, diff_torch)
    # knowledge_state_df = pd.DataFrame(state.cpu().numpy(), columns=['kcs'])
    ...
    # knowledge_state_df['difficulty'] = kt_model.ids_difficulty.view(-1).cpu().numpy()

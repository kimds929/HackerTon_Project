...
while(i<n_iter):
    ...
    if i == 0:
        ...
        kc = git.generate(aa.grade, size=1, return_type='kc')

    diff = np.random.normal(diff_mean, diff_std, size=len(kc))

    aa.solve_simulation(kc, diff)
    st = aa.get_knowledge_state()
    knowledge_hist.append(st.mean())

    i += 1
    pbar.set_postfix(knowledge_level=f"{st.mean().item():.2f}")
    pbar.update(1)
    
pbar.close()

# cPickle.dump([k.item() for k in knowledge_hist], open(f'{dataset_path}/agent_random_policy1(mean_0,std_1)2.pkl', 'wb'))

plt.plot(knowledge_hist, 'o-')
